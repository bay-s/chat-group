...
<p className='lh-base has-text-grey-light'>{data.server_description}</p>
{
    data.creator_id === value.data.user_id ? <p className='has-text-grey-light'>This is the start of the #general</p>
: ''
}
{
    data.creator_id === value.data.user_id ? <Link to={`/channel/edit/${data.id}`} className='button is-dark is-outlined has-text-info'>
<i class="fa fa-pencil has-text-info pr-3" aria-hidden="true"></i>
Edit Channel
</Link> 
: ''
}
